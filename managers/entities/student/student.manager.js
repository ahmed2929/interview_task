...
class StudentManager { 
     ...
     */
    async create({firstName, lastName, classroomId,schoolId,username,password, __longToken}){
        try {
            ...
            const student = {firstName, lastName, classroomId,username,password,schoolId};
     ...
     */
    async update({firstName, lastName, classroomId, schoolId,id,username, __longToken}){
        try {
            const userRole = __longToken?.role;
            ...
            const student = {firstName, lastName, classroomId, id,schoolId,username};
    ...
    async delete({__query, __longToken}){
        try {
            ...
            const studentExists = await this.mongomodels.student.findById(id);
            if(!studentExists) return {error: 'student not found', status: 404, ok: false};
            if(userRole===ROLES.SCHOOL_ADMIN && studentExists.schoolId!==__longToken.schoolId) return {error: 'Unauthorized', status: 401, ok: false};
